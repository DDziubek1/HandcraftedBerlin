...
export const createCategory = asyncHandler(async (req, res) => {
    ...
    const { name } = req.body;
    const found = await Cart.findOne({ name });
    if (found) throw new ErrorResponse("Category already exists in DB", 400);

    const category = await Cart.create(req.body);
...
export const deleteCategory = asyncHandler(async (req, res) => {
    const { id } = req.params;
    const category = await Cart.findByIdAndDelete(id);
...
export const getCategoryByName = asyncHandler(async (req, res) => {
    ...
    const category = await Cart.find({ name });
